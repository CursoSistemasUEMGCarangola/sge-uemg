...
-- 2. Wipe Public Schema AND Auth Users (Clean Slate)
-- We drop tables to remove dependencies, then clean auth, then recreate.
...
-- Now that public tables are gone, satisfy the user request to clean old auth records
-- WARNING: This deletes ALL users.
DELETE FROM auth.users;

-- 3. Re-Create Schema (Copy of schema.sql)
...
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    crypt('123456', gen_salt('bf')), -- Uses pgcrypto to hash '123456'
    NOW(),
    NOW(),
    NOW(),
